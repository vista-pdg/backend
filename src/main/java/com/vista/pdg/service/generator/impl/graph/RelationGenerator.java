...
public class RelationGenerator implements StructureGenerator<RelationContract> {
    ...
    private List<List<Object>> computePairs(RelationContract contract, List<Object> set) {
        ...
        for (Object a : set) for (Object b : set) if (satisfies(contract.rule(), a, b)) result.add(List.of(a, b));
        return result;
    }
    ...
    private Map<String, Object> computeProperties(RelationContract contract,
                                                    List<Object> set,
        ...
        boolean isEquivalence = Boolean.TRUE.equals(result.get("reflexive"))
    ...
    private boolean isAntisymmetric(List<List<Object>> pairs, Set<String> pairSet) {
        return pairs.stream().allMatch(p -> p.get(0).equals(p.get(1)) || !pairSet.contains(p.get(1) + ":" + p.get(0)));
    }

    private boolean isTransitive(List<List<Object>> pairs, Set<String> pairSet) {
        for (List<Object> p1 : pairs)
            for (List<Object> p2 : pairs)
                if (p1.get(1).equals(p2.get(0)) && !pairSet.contains(p1.get(0) + ":" + p2.get(1))) return false;
        return true;
    }
