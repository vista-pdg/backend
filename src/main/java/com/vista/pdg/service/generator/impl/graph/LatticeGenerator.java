...
public class LatticeGenerator implements StructureGenerator<LatticeContract> {
  ...
  private GeneratedStructure buildDivisors(LatticeContract contract) {
    ...
    for (int d : divs)
      nodes.add(new Node3D(idMap.get(d), String.valueOf(d), 0, 0, 0, 0, null, Map.of()));
    ...
    for (int a : divs) {
      for (int b : divs) {
        if (b > a && b % a == 0 && isPrime(b / a)) {
          boolean direct =
              divs.stream().noneMatch(c -> c != a && c != b && b % c == 0 && c % a == 0);
          if (direct)
            edges.add(new Edge3D("e" + edgeIdx++, idMap.get(a), idMap.get(b), null, true));
        }
      }
    }
      ...
      for (List<String> pair : contract.order()) {
        if (pair.size() == 2) {
          ...
          if (from != null && to != null)
            edges.add(new Edge3D("e" + edgeIdx++, from, to, null, true));
        }
      }
    }
    return new GeneratedStructure(contract, nodes, edges, Map.of());
