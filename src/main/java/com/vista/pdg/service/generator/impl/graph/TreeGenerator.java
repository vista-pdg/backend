...
public class TreeGenerator implements StructureGenerator<TreeContract> {
    ...
    private AvlNode minNode(AvlNode n) { return n.left == null ? n : minNode(n.left); }
    ...
    private void avlDfs(AvlNode n, String parentId, int depth,
                         List<Node3D> nodes, List<Edge3D> edges, int[] idx) {
        if (n == null) return;
        String id = "n" + idx[0]++;
    ...
    private void heapInsert(List<Integer> heap, int v) {
        ...
        while (i > 0) {
            int parent = (i - 1) / 2;
            if (heap.get(parent) < heap.get(i)) { Collections.swap(heap, parent, i); i = parent; }
            else break;
        }
    }
    ...
    private GeneratedStructure buildFromNodes(TreeContract contract) {
        if (contract.nodes() == null) return new GeneratedStructure(contract, List.of(), List.of(), Map.of());
        Map<String, Integer> depthMap = new HashMap<>();
        List<Node3D> nodes = new ArrayList<>();
        List<Edge3D> edges = new ArrayList<>();
        for (TreeContract.NodeDef nd : contract.nodes()) {
            int depth = nd.parent() == null ? 0 : depthMap.getOrDefault(nd.parent(), 0) + 1;
            depthMap.put(nd.id(), depth);
            nodes.add(new Node3D(nd.id(), String.valueOf(nd.value()), 0, 0, 0, depth, nd.parent(), Map.of()));
            if (nd.parent() != null) edges.add(new Edge3D("e" + edges.size(), nd.parent(), nd.id(), null, true));
        }
        return new GeneratedStructure(contract, nodes, edges, Map.of());
    }
}
